...
parser.add_argument('Input', type=str, help="Specifies the text to count from, if the -f (--file) flag is supplied, will treat input as a filename.")

parser.add_argument('N', type=int, help="Specifies that you want the N^th most used word. If zero, will return the count of all words.")
...
# Argument Checks
if args.N < 0 :
    raise ValueError(f"Argument N supplied ({args.N}) is invalid, N must be at least zero.")
...
def WordDictToList(dictWords : dict) :
    ...
    sorted_list = sorted(lWords, key=lambda x: (x[0], x[1]),reverse=True) # Sorting tuples by first and second item.
    return sorted_list
...
def ParseText(sText : str) :
    ...
    lUnprocessedWordList = RemoveAllNonAlphanumeric(sText.strip().strip("\n").replace("\t"," ")).split(" ")
...
def main() :
    ...
    if args.N == 0 :
        for i in range(len(lWordCount)) :
            print(f'The {i+1}{GetNumberSuffix(i)} most used word in the text is : "{lWordCount[i][1]}" with {lWordCount[i][0]} occurences.')


    else :
        print(f'The {args.N}{GetNumberSuffix(args.N)} most used word in the text is : "{lWordCount[args.N-1][1]}" with {lWordCount[args.N-1][0]} occurences. ({len(lWords)} words in text)')
